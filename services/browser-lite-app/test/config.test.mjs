...
test("returning from an instance shrinks into its exact Space card", async () => {
  ...
  assert.match(css, /\.browser-surface\s*\{[^}]*transform-origin:0 0/);
  assert.match(css, /\.space-return-target \.space-preview\s*\{\s*visibility:hidden/);
  assert.match(renderer, /createBrowserSurface\(space, installation, safePreview\(space\), "space-card-surface"\)/);
  assert.match(renderer, /querySelectorAll\("\[id\], \[data-action\], button, input, form, \[tabindex\]"\)/);
  assert.match(css, /\.browser-surface \.browser-surface-chrome\s*\{[^}]*display:block/);
});
