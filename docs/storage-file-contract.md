...
```json
{
  ...
  "file": {
    ...
    "type": "image/png",
    "name": "screenshot.png",
    "url": "https://files.example.com/browser-pilot/files/....png?X-Amz-Expires=900&...",
    "storage": "s3"
  },
  "screenshot": null
}
...
Base64 截图只能作为兼容模式或显式 `includeBase64=true` 使用，不能作为默认最终结果。旧的 `store` query 参数已废弃；截图现在总是写入 FileStore。
...
- 文件 URL 是 15 分钟有效的完整签名 URL；S3 模式必须使用公开可达的 S3 预签名 URL，Builtin 模式使用后端签名 URL。
...
- 截图 API 默认写入 FileStore，默认 `screenshot: null`；`includeBase64=true` 才返回 base64 兼容旧客户端。
- CLI 默认请求 `includeBase64=false`，输出文件记录和签名 `file.url`，不会打印大段 base64。
- CLI `-o` 先让后端写入 FileStore，再通过签名 `file.url` 下载一个本地副本。
...
- 用户侧 `POST/GET/PATCH/DELETE /api/sessions/{sessionId}/files...` 支持上传、读取元数据、重命名和删除已完成文件；删除响应区分对象删除和 DB 记录删除，存储对象删除失败时仍可移除列表记录并返回 warning。
- 删除 Session 时必须让用户看到关联文件并选择处理方式；未选择删除的已完成文件会归档到全局 Files 页面，已选择文件会删除对象并移除记录。
- 用户级 `GET/PATCH/DELETE /api/files...` 管理全局文件；member 只访问自己的文件，admin/superadmin 访问同租户文件；会话级 Token 不能访问全局文件管理，也不能重新获取已归档文件 URL。已签发的短期下载 URL 在 15 分钟内是 capability URL。
- `bpilot files list --json` 对外暴露同一个统一文件列表，不做 cursor、未读状态或动作级下载推理。
- `bpilot files upload/get/rename/delete` 只处理明确的文件管理动作。
- 文件下载 URL 统一为 15 分钟签名 URL；S3 模式返回 S3 预签名 URL，Builtin 模式返回后端签名代理 URL。
