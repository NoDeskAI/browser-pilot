...
`./start.sh prod` 已经有意移除。single-host 只用于 CE / private / self-host 单机部署；EE SaaS production 只走 Kubernetes 部署路径。

EE SaaS Kubernetes 部署入口在 `deploy/ee-saas/`：
...
# 从 platform 控制面同步 approved images / tenant namespaces / quota，再用 Helm reconciliation
...
values 示例见 `deploy/ee-saas/values.example.yaml`。EE SaaS 部署必须提供 backend 镜像 digest、approved runtime image digest、tenant runtime namespace 和必要 Secret；脚本会在 `plan/apply` 阶段校验 digest 必须是 `sha256:<64 hex>`，拒绝 placeholder 和 floating tag。platform API 会导出只包含 runtime approved images、tenant namespace 和 quota 派生 ResourceQuota 的覆盖 values，避免手工拼 tenant namespace 基线；`verify` 会检查集群里的 backend SaaS env、admission policy、tenant namespace 基线和 unsafe session Pod 拒绝路径。tenant delete 是 platform 控制面里的软删除，会写入保留期；保留期结束后只能由 platform admin 显式登记 purge request，并写入 platform audit。真正执行 DB/file object purge 前还会检查没有未结束 runtime placement；K8s namespace 和集群残留资源清理由 EE provider/controller 负责。
...
| `BROWSER_RUNTIME_PROVIDER` | `docker`                                                 | runtime provider 选择器。`kubernetes` 预留给 EE SaaS；EE provider 不存在时必须 fail closed。 |
| `EE_SAAS_MODE`        | `false`                                                        | 开启 SaaS 专用后端闸门：tenant Browser Images API 被拒绝、tenant 传入 runtime image 被阻断，并启用 tenant runtime quota/status 检查。 |
| `SAAS_DEFAULT_ACTIVE_SESSION_LIMIT` | `3`                                           | 创建 SaaS tenant 且未显式设置 quota 时写入的默认 active session quota。 |
| `SAAS_DEFAULT_MAX_SESSION_SECONDS` | `3600`                                          | 写入 SaaS tenant runtime quota 的默认最长 session 时长，单位秒。 |
...
| `BROWSER_RUNTIME_COMMAND_MAX_TIMEOUT` | `3600` | runtime-worker Docker 命令允许的最大超时时间，单位秒。`EE_SAAS_MODE=true` 时 runtime shell command 会被后端直接拒绝。 |
...
single-host Docker Compose 部署只在宿主机公开内置 Nginx 反向代理的 80/443。浏览器容器操作通过内部 `runtime-worker` 服务执行；公网 backend 只通过 Compose 私有网络和 `BROWSER_RUNTIME_CONTROL_TOKEN` 访问 worker，只有 worker 挂载 `/var/run/docker.sock`。不要发布 worker 端口，公网部署前必须把 runtime control token 改成长随机值。EE SaaS production 不走这条 Compose 入口，后续只走 Kubernetes。
