...
Session 文件统一由后端 FileStore 管理。用户和会话级 API Token 可以通过 `/api/sessions/{sessionId}/files` 列出、上传、读取元数据、重命名和删除文件；删除响应会区分后端对象删除和列表记录删除。文件内容 URL 继续走后端 `/api/files/...` 代理，不暴露 S3 内部 endpoint。
